...
CATEGORY_SOURCES = {
    ...
    "ETF": [
        ...
        "https://seekingalpha.com/market-outlook/etfs.xml"
    ],
    "黃金": [
...
def get_today_category():
    ...
    mapping = {0: "台股", 1: "幣圈", 2: "美股", 3: "幣圈", 4: "ETF", 5: "黃金"}
    return mapping.get(weekday, "台股")
...
def summarize_with_gpt(news):
    prompt = f"""
你是一位財經新聞編輯，請根據以下新聞標題與摘要，撰寫一篇全新、自然、有條理、約 500～1000 字的中文財經新聞，避免抄襲，語氣自然易讀，可補充背景與分析觀點。
...
def main():
    ...
    # 圖片生成（佔位）
    image_path = get_next_image_filename()
    os.makedirs(os.path.dirname(image_path), exist_ok=True)
    with open(image_path, "wb") as f:
        f.write(b"FAKE IMAGE CONTENT")  # 可改為圖像生成

    print(f"✅ 新聞已更新，圖片：{image_path}")
